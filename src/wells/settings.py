...
interactive CLI menu (``wells config`` or the top-level menu when
...
Design goals:
  ...
  * Live + persistent — updated values are written to os.environ AND .env, so
    a subsequent ``wells <goal>`` in the same shell picks them up too.
"""
...
SETTINGS: list[Setting] = [
    ...
    Setting(
        ...
        "Capabilities",
        "Discover skills/<name>/SKILL.md and expose load_skill (on/off).",
        "1",
        choices=("0", "1"),
    ),
    Setting(
        ...
        "CodeAct (run_code)",
        "Capabilities",
        "Expose the sandboxed run_code tool for in-context computation.",
        "1",
        choices=("0", "1"),
    ),
    Setting(
        ...
        "Background agents",
        "Capabilities",
        "Expose bg_start/bg_status/bg_collect for concurrent fan-out.",
        "1",
        choices=("0", "1"),
    ),
    Setting(
