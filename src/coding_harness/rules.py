...
rules:
  - id: gpu-rental-teardown
    severity: liability
    open: '\b(vastai|vast\.ai)\s+(create|launch)|\brunpod(ctl)?\s+(create|start|resume)|\blambda\s+cloud.*(launch|start)|\bgcloud\s+compute\s+instances\s+create|\baws\s+ec2\s+run-instances|\baz\s+vm\s+create'
    close: '\b(vastai|vast\.ai)\s+(destroy|stop)|\brunpod(ctl)?\s+(remove|stop|terminate)|\blambda\s+cloud.*(terminate|delete)|\bgcloud\s+compute\s+instances\s+(delete|stop)|\baws\s+ec2\s+(terminate|stop)-instances|\baz\s+vm\s+(delete|deallocate)'
    message: >-
